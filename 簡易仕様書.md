...
### 現状の懸念点
- 非ログインユーザーにも、ログイン済みでしか使えない操作のUIが表示される
  - `authToken`に任意の数字・文字列を追加することで表示される
- 表示されていても、実際にDBから情報を取得する操作はAPIサーバー側でブロックされる
...
| サーバー側で認証状態を判定する `/api/auth` を利用する    | ログイン状態の判定を確実に行う |

### 改善方針
- authTokenの有無のみでログイン済みかの判定を行うのは不正確
- APIを用いた認証状態の確認と、それに応じたUIの切り替えを実装する